...
class AbstractController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	protected function getAddressTitles() {
		...
		foreach ($values as $value) {
			$title = new \stdClass();
			$title->value = $value;
			$title->label = LocalizationUtility::translate('tx_clubdirectory_domain_model_address.title.' . $value, 'clubdirectory');
			$titles[] = $title;
		}
		return $titles;
	}
	...
	protected function deleteUploadedFilesOnValidationErrors($argument) {
		if ($this->getControllerContext()->getRequest()->hasArgument($argument)) {
			...
			if ($club instanceof \JWeiland\Clubdirectory\Domain\Model\Club) {
				...
				if (count($images)) {
					/** @var \JWeiland\Clubdirectory\Domain\Model\FileReference $image */
